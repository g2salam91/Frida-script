...
Java.perform(function() {
    ...
    Java.use("android.os.Build").MANUFACTURER.value = "samsung-Pentest1337";
    Java.use("android.os.Build").BRAND.value = "samsung-Pentest1337";
    ...
    // Combined File.exists hook for both emulator and root artifacts
    try {
        Java.use("java.io.File").exists.implementation = function() {
            var name = Java.use("java.io.File").getName.call(this);
            // Check for emulator artifacts first
            var emulatorCatched = ["qemud", "qemu_pipe", "drivers", "cpuinfo"].indexOf(name) > -1;
            if (emulatorCatched) {
                console.log("the pipe " + name + " existence is hooked (emulator)");
                return false;
            }
            // Then check for root artifacts
            var rootCatched = commonPaths.indexOf(this.getAbsolutePath()) > -1;
            if (rootCatched) {
                 console.log("Bypassing Root Check for file: " + name);
                 return false;
            }
    ...
    // rename the package names (emulator specific)
    try {
        Java.use("android.app.ApplicationPackageManager").getPackageInfo.overload(
            ...
            var catched = ["com.example.android.apis", "com.android.development"].indexOf(name) >
                -1;
            if (catched) {
                console.log("the package " + name + " is renamed with fake name (emulator)");
    ...
    try {
        Interceptor.attach(Module.findExportByName(null, "android_getCpuFamily"), {
            onLeave: function(retval) {
                ...
                if ([2, 5].indexOf(retval.toInt32()) > -1) { // Ensure retval is treated as int
...
setTimeout(function() {
    ...
    function bypassNativeFileCheck() {
        ...
        Interceptor.attach(fopen, {
            ...
            onLeave: function(retval) {
                if (retval.toInt32() != 0) { // File was opened successfully
                    if (commonPaths.indexOf(this.inputPath) >= 0) {
                        console.log("Anti Root Detect - fopen : " + this.inputPath);
                        retval.replace(ptr(0x0)); // Return NULL file pointer
                    }
                }
            },
        });
        ...
        Interceptor.attach(access, {
            ...
            onLeave: function(retval) {
                if (retval.toInt32() == 0) { // File exists (access returned 0)
                    if (commonPaths.indexOf(this.inputPath) >= 0) {
                        console.log("Anti Root Detect - access : " + this.inputPath);
                        retval.replace(ptr(-1)); // Return -1 (error/doesn't exist)
                    }
                }
            },
        });
    ...
    function setProp() {
        ...
        Interceptor.attach(system_property_get, {
            ...
            onLeave(ret) {
                ...
                // Added check for ro.debuggable and ro.secure in native layer
    ...
    function bypassRootAppCheck() {
        ...
        // Combined hook for root management apps using the main ROOTmanagementApp list
    ...
    bypassShellCheck();
    Java.perform(function() {
        // Merged RootPackages list (original RootPackages + ROOTmanagementApp + extra items)
        var MergedRootPackages = [
            "com.noshufou.android.su",
            "com.noshufou.android.su.elite",
            "eu.chainfire.supersu",
            "com.koushikdutta.superuser",
            ...
            "com.ramdroid.appquarantine",
            "com.ramdroid.appquarantinepro",
            "com.devadvance.rootcloak", // Added from original RootPackages
            "com.devadvance.rootcloakplus", // Added from original RootPackages
            "de.robv.android.xposed.installer", // Added from original RootPackages
            "com.saurik.substrate", // Added from original RootPackages
            "com.zachspong.temprootremovejb", // Added from original RootPackages
            "com.amphoras.hidemyroot", // Added from original RootPackages
            "com.amphoras.hidemyrootadfree", // Added from original RootPackages
            "com.formyhm.hiderootPremium", // Added from original RootPackages
            "com.formyhm.hideroot", // Added from original RootPackages
            "me.phh.superuser", // Added from original RootPackages
            "eu.chainfire.supersu.pro", // Added from original RootPackages
            "com.kingouser.com", // Added from original RootPackages
            "com.topjohnwu.magisk", // Present in ROOTmanagementApp
            "me.weishu.kernelsu", // Present in ROOTmanagementApp
        ];
        ...
        var NativeFile = Java.use("java.io.File"); // Renamed from 'File' to avoid confusion if needed later
        var String = Java.use("java.lang.String");
        ...
        // Use the merged list for the PackageManager hook
        PackageManager.getPackageInfo.overload(
            "java.lang.String",
            "int"
        ).implementation = function(pname, flags) {
            var shouldFakePackage = MergedRootPackages.indexOf(pname) > -1;
        ...
        // The File.exists hook is now handled at the top level, so this specific hook is removed
        // Native file checks for RootBinaries are handled by bypassNativeFileCheck()

        // Hooks for Runtime.exec (Removed the redundant set)
        ...
        // ProcessBuilder.start hook (Removed the duplicate)
